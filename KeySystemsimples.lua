-- ==============================================================================
--         SISTEMA DE PROTEÇÃO EMBUTIDO LOWHIGH STORE - SIMPLES (LEVEL 1)
-- ==============================================================================
...
local function CalcularTempoRestante(expiryTimestamp)
    ...
    if diff > 157680000 then return "Vitalício"
    elseif diff > 25920000 then return "1 Year"
    elseif diff > 2160000 then return "1 Month"
    elseif diff > 432000 then return "1 Week"
    elseif diff > 0 then return "1 Day"
    else return "Expirado" end
end
...
local KeyAuthApp = KeyAuthClass.new(
    ...
    "bg8cRvXsrd",
    "803735a5e270d0b55245276f24f541361b0065532fb94760c86688ac90ddb9dd", 
    "1.0"
)
...
local TextBox = Instance.new("TextBox", LoginFrame); TextBox.Size = UDim2.new(0, 260, 0, 35); TextBox.Position = UDim2.new(0.5, -130, 0.4, -10); TextBox.BackgroundColor3 = Color3.fromRGB(25, 25, 25); TextBox.Text = ""; TextBox.PlaceholderText = "Cole sua Key aqui... (Simples)"; TextBox.TextColor3 = Color3.fromRGB(220, 220, 220); TextBox.Font = Enum.Font.Gotham; TextBox.TextSize = 12; Instance.new("UICorner", TextBox)
...
Btn.MouseButton1Click:Connect(function()
    ...
    if status then
        -- VERIFICANDO SE A KEY É DO NÍVEL 1 (SIMPLES)
        if KeyAuthApp.user_data.subscriptions[1].level == "1" then
            ...
            print("Acesso SIMPLES Liberado! Duração: " .. expiryStr)
            
            carregarScriptSimple(expiryStr) -- Aciona a função abaixo
        else
            game.Players.LocalPlayer:Kick("❌ LowHigh Store: Acesso NEGADO. Esta key não é do nível SIMPLES (Level 1)!")
        end
    else
...
-- ==============================================================================
function carregarScriptSimple(duracaoDaKey)
    
-- ==============================================================================
--         SCRIPT SIMPLES (LEVEL 1) - AIMBOT LEGIT E ESP OTIMIZADO
-- ==============================================================================
    ...
    -- Apenas as 2 imagens necessárias (Logo, Mouse e Olho)
    local ImageURIs = {
    ...
    -- Configurações globais (Apenas o que tem no Simples)
    ...
    pcall(function()
        ...
        if LocalPlayer:FindFirstChild("PlayerGui") and LocalPlayer.PlayerGui:FindFirstChild(guiNameUI) then
            LocalPlayer.PlayerGui[guiNameUI]:Destroy()
        end
    end)
    ...
    MyLogo.Size = UDim2.new(0, 60, 0, 60); MyLogo.Position = UDim2.new(0.5, -30, 0, 8); MyLogo.Image = "rbxthumb://type=Asset&id=125256544092304&w=150&h=150"; MyLogo.BackgroundTransparency = 1; MyLogo.ScaleType = Enum.ScaleType.Stretch; Instance.new("UICorner", MyLogo).CornerRadius = UDim.new(1, 0) 
    ...
    UserLabel.Position = UDim2.new(1, -165, 0, 12); UserLabel.Size = UDim2.new(0, 150, 0, 20); UserLabel.Text = LocalPlayer.DisplayName; UserLabel.TextColor3 = Theme.Accent; UserLabel.Font = Enum.Font.GothamBold; UserLabel.TextSize = 14; UserLabel.TextXAlignment = Enum.TextXAlignment.Right; UserLabel.BackgroundTransparency = 1

    local StatusLabel = Instance.new("TextLabel", Header)
    StatusLabel.Position = UDim2.new(1, -165, 0, 32); StatusLabel.Size = UDim2.new(0, 150, 0, 15); StatusLabel.Text = duracaoDaKey or "Vitalício"; StatusLabel.TextColor3 = Color3.fromRGB(255, 255, 255); StatusLabel.Font = Enum.Font.Gotham; StatusLabel.TextSize = 11; StatusLabel.TextXAlignment = Enum.TextXAlignment.Right; StatusLabel.BackgroundTransparency = 1
    ...
    local function CreateTab(Name, ImageID, TamanhoX, TamanhoY)
        ...
        TabWrapper.Size = UDim2.new(1, 0, 0, 42); TabWrapper.BackgroundTransparency = 1
        
        local HitboxBtn = Instance.new("TextButton", TabWrapper)
        HitboxBtn.Size = UDim2.new(1, 0, 1, 0); HitboxBtn.BackgroundTransparency = 1; HitboxBtn.Text = ""; HitboxBtn.ZIndex = 5
        
        local TabIcon = Instance.new("ImageLabel", TabWrapper)
        TabIcon.Size = UDim2.new(0, TamanhoX, 0, TamanhoY); TabIcon.Position = UDim2.new(0.5, -TamanhoX/2, 0.5, -TamanhoY/2); TabIcon.BackgroundTransparency = 1; TabIcon.Image = "rbxthumb://type=Asset&id=" .. tostring(ImageID) .. "&w=150&h=150"; TabIcon.ScaleType = Enum.ScaleType.Stretch; TabIcon.ImageColor3 = Theme.DarkText
        
        local TabIndicator = Instance.new("Frame", TabWrapper)
        TabIndicator.Size = UDim2.new(0, 3, 0.8, 0); TabIndicator.Position = UDim2.new(0, 0, 0.1, 0); TabIndicator.BackgroundColor3 = Theme.Accent; TabIndicator.BorderSizePixel = 0; TabIndicator.Visible = false

        local Page = Instance.new("Frame", PageContainer)
        Page.Size = UDim2.new(1, 0, 1, 0); Page.BackgroundTransparency = 1; Page.Visible = false
        
        local LeftCol = Instance.new("ScrollingFrame", Page)
        LeftCol.Size = UDim2.new(0.48, 0, 1, 0); LeftCol.BackgroundTransparency = 1; LeftCol.ScrollBarThickness = 0; local LeftLayout = Instance.new("UIListLayout", LeftCol); LeftLayout.Padding = UDim.new(0, 8)
        
        local RightCol = Instance.new("ScrollingFrame", Page)
        RightCol.Size = UDim2.new(0.48, 0, 1, 0); RightCol.Position = UDim2.new(0.52, 0, 0, 0); RightCol.BackgroundTransparency = 1; RightCol.ScrollBarThickness = 0; local RightLayout = Instance.new("UIListLayout", RightCol); RightLayout.Padding = UDim.new(0, 8)
    ...
    local function CreateToggle(Parent, Name, Default, Callback)
        ...
        Checkbox.MouseButton1Click:Connect(function()
            State = not State; Checkbox.BackgroundColor3 = State and Theme.Accent or Theme.ToggleOff; CheckIcon.Visible = State; Callback(State)
        end)
    end
    ...
    local function CreateDropdown(Parent, Name, Options, DefaultIndex, Callback)
        ...
        local MainBtn = Instance.new("TextButton", Frame); MainBtn.Size = UDim2.new(1, 0, 0, 20); MainBtn.Position = UDim2.new(0, 0, 0, 18); MainBtn.BackgroundColor3 = Theme.ToggleOff; MainBtn.Text = "  " .. Options[DefaultIndex]; MainBtn.TextColor3 = Theme.Text; MainBtn.Font = Enum.Font.Gotham; MainBtn.TextSize = 10; MainBtn.TextXAlignment = Enum.TextXAlignment.Left; Instance.new("UICorner", MainBtn).CornerRadius = UDim.new(0, 4)
        ...
        local DropContainer = Instance.new("Frame", Frame); DropContainer.Size = UDim2.new(1, 0, 0, #Options * 20); DropContainer.Position = UDim2.new(0, 0, 0, 40); DropContainer.BackgroundColor3 = Color3.fromRGB(20, 20, 20); Instance.new("UICorner", DropContainer).CornerRadius = UDim.new(0, 4)
        local DropLayout = Instance.new("UIListLayout", DropContainer)

        local IsOpen = false
        MainBtn.MouseButton1Click:Connect(function()
            IsOpen = not IsOpen; Arrow.Text = IsOpen and "▲" or "▼"; Frame.Size = IsOpen and UDim2.new(1, -15, 0, 40 + (#Options * 20) + 2) or UDim2.new(1, -15, 0, 40)
        end)
        ...
        for i, opt in pairs(Options) do
            ...
            OptBtn.MouseButton1Click:Connect(function()
                IsOpen = false; Arrow.Text = "▼"; Frame.Size = UDim2.new(1, -15, 0, 40); MainBtn.Text = "  " .. opt
                for _, btn in pairs(DropContainer:GetChildren()) do if btn:IsA("TextButton") then btn.TextColor3 = Theme.Text end end
                OptBtn.TextColor3 = Theme.Accent; Callback(opt)
            end)
        end
    end
    ...
    local function CreateSlider(Parent, Name, Min, Max, Default, Callback, Suffix)
        ...
        local SliderBg = Instance.new("Frame", Frame); SliderBg.Size = UDim2.new(1, 0, 0, 10); SliderBg.Position = UDim2.new(0, 0, 0, 22); SliderBg.BackgroundColor3 = Color3.fromRGB(8, 8, 8); SliderBg.BorderSizePixel = 0; Instance.new("UICorner", SliderBg).CornerRadius = UDim.new(0, 3) 
        local Fill = Instance.new("Frame", SliderBg); Fill.Size = UDim2.new((Default - Min) / (Max - Min), 0, 1, 0); Fill.BackgroundColor3 = Theme.Accent; Fill.BorderSizePixel = 0; Instance.new("UICorner", Fill).CornerRadius = UDim.new(0, 3)
        local Trigger = Instance.new("TextButton", SliderBg); Trigger.Size = UDim2.new(1, 0, 1, 0); Trigger.Position = UDim2.new(0,0,0,0); Trigger.BackgroundTransparency = 1; Trigger.Text = ""
        ...
        ValInput.FocusLost:Connect(function()
            ...
            if inputVal then
                local clampedVal = math.clamp(inputVal, Min, Max); local pct = (clampedVal - Min) / (Max - Min); Fill.Size = UDim2.new(pct, 0, 1, 0); ValInput.Text = tostring(clampedVal) .. Suffix; Callback(clampedVal)
            else
                local currentPct = Fill.Size.X.Scale; local currentVal = math.floor(Min + ((Max - Min) * currentPct)); ValInput.Text = tostring(currentVal) .. Suffix
            end
        end)
    end
    ...
    RunService.Heartbeat:Connect(function()
        if ActiveSlider then
            ...
            local Val = math.floor(ActiveSlider.Min + ((ActiveSlider.Max - ActiveSlider.Min) * Pct)); ActiveSlider.Fill.Size = UDim2.new(Pct, 0, 1, 0); ActiveSlider.ValLabel.Text = tostring(Val) .. ActiveSlider.Suffix; ActiveSlider.Callback(Val)
        end
    end)

    -- Apenas 2 Abas: AIM e ESP
    local L1, R1 = CreateTab("AIM", 128137609286733, 14, 22)  
    local L2, R2 = CreateTab("ESP", 90674527474660, 26, 15)   

    -- ABA 1: COMBAT E AIMBOT SETTINGS
    ...
    CreateDropdown(R1, "Type Aimbot", {"Aimbot Legit"}, 1, function(val) _G.AimbotType = val end)
    ...
    local function GetPredictedPosition(Target)
        ...
        local TotalTime = (Distance / _G.BulletSpeed) + _G.CurrentPing
        local CompensationForce = Vector3.new(0, (_G.BulletDrop * 196.2), 0)
        return TargetPart.Position + ((TargetPart.AssemblyLinearVelocity or Vector3.new(0,0,0)) * TotalTime) + (0.5 * CompensationForce * (TotalTime ^ 2))
    end
    ...
    for _, player in pairs(Players:GetPlayers()) do if player ~= LocalPlayer then CreateESPObj(player) end end
    Players.PlayerAdded:Connect(function(player) if player ~= LocalPlayer then CreateESPObj(player) end end)
    Players.PlayerRemoving:Connect(function(player)
        if ESP_Table[player] then
            ...
            ESP_Table[player].boxNormal:Remove(); ESP_Table[player].name:Remove(); ESP_Table[player].distance:Remove()
            ESP_Table[player].hpOutline:Remove(); ESP_Table[player].hpBar:Remove(); ESP_Table[player].tracer:Remove()
    ...
    RunService:BindToRenderStep("EliteHubMain", Enum.RenderPriority.Camera.Value + 1, function()
        ...
        for player, drawings in pairs(ESP_Table) do
            ...
            if char and char:FindFirstChild("Humanoid") and char.Humanoid.Health > 0 and char:FindFirstChild("HumanoidRootPart") then
                ...
                if TopVis and BotVis then
                    ...
                    for _, l in pairs(drawings.corners) do l.Visible = false end; for _, l in pairs(drawings.skeleton) do l.Visible = false end; drawings.boxNormal.Visible = false; drawings.name.Visible = false; drawings.distance.Visible = false; drawings.hpOutline.Visible = false; drawings.hpBar.Visible = false; drawings.tracer.Visible = false 
                end
            else 
                for _, l in pairs(drawings.corners) do l.Visible = false end; for _, l in pairs(drawings.skeleton) do l.Visible = false end; drawings.boxNormal.Visible = false; drawings.name.Visible = false; drawings.distance.Visible = false; drawings.hpOutline.Visible = false; drawings.hpBar.Visible = false; drawings.tracer.Visible = false 
            end
        ...
        if _G.AimbotEnabled then
            ...
            if CachedTarget and CachedTarget.Character then
                CachedAimbotPos = GetPredictedPosition(CachedTarget)
            else
                CachedAimbotPos = nil
            end
        else
        ...
        if _G.AimbotEnabled and CachedTarget and CachedAimbotPos then
            ...
            if _G.Smoothness >= 1 then Camera.CFrame = TargetCF else Camera.CFrame = Camera.CFrame:Lerp(TargetCF, _G.Smoothness) end
        end
    end)
